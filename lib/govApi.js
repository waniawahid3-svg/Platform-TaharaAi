...
// The whole master framework: every control by framework, with verdict, evidence status and percentages.
export async function getMaster(engagementId) {
  const res = await fetch(`${GOV_API_BASE}/engagements/${engagementId}/master`);
  return asJson(res);
}
